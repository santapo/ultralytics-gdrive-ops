...
class TrainManager:
    ...
    def _initialize(self):
        ...
        """
        self.current_local_dataset_paths = self._check_for_new_files(self.local_dataset_path, [])
        self.current_gdrive_dataset_paths = self._check_for_new_files(self.gdrive_dataset_path, [])
        self.current_local_model_logs = self._check_for_new_files(self.local_model_logs_path, [])
        self.current_gdrive_model_logs = self._check_for_new_files(self.gdrive_model_logs_path, [])
    ...
    def start_monitoring(self):
        ...
        while True:
            ...
            new_dataset_files = check_for_new_files(self.gdrive_dataset_path, self.current_dataset_paths)
            ...
            dataset_path = self.training_queue.pop(0)
            downloaded_path = download_file(dataset_path, self.local_dataset_path)

            # step 3 prepare the dataset for training in the local dataset path
            dataset_path = Trainer.prepare_dataset(downloaded_path)
    ...
    def _sync_model_logs(self):
        ...
        """
        new_model_logs = check_for_new_files(self.local_model_logs_path, self.current_model_logs)
...
if __name__ == "__main__":
    ...
    train_manager.start_monitoring()